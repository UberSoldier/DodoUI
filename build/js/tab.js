     ...
     */
     Tab.fn.render = function() {
        ...
        if (_config.elem === undefined) {
            layui.hint().error('Tab error: 请配置选项卡容器.');
            return that;
        }
     ...
     var _tab = {
        ...
        tabDomExists: function() {
            var that = this;
            if (_doc.find('div.dodo-tab').length > 0) {
        ...
        createTabDom: function() {
            ...
            // 模板
            var tpl = [
                '<div class="layui-tab layui-tab-card dodo-tab" lay-filter="' + that._filter + '">',
                '<ul class="layui-tab-title">',
                '<li class="layui-this" lay-id="-1" data-url="' + _config.mainUrl + '"><i class="layui-icon">&#xe68e;</i> 控制面板</li>',
                '</ul>',
                '<div class="dodo-tab-tool">操作&nbsp;<i class="fa fa-caret-down"></i></div>',
                '<div class="dodo-tab-tool-body layui-anim layui-anim-upbit">',
                '<ul>',
                '<li class="dodo-item" data-target="refresh">刷新当前选项卡</li>',
                '<li class="dodo-line"></li>',
                '<li class="dodo-item" data-target="closeCurrent">关闭当前选项卡</li>',
                '<li class="dodo-item" data-target="closeOther">关闭其他选项卡</li>',
                '<li class="dodo-line"></li>',
                '<li class="dodo-item" data-target="closeAll">关闭所有选项卡</li>',
                '</ul>',
                '</div>',
            ...
            //监听操作定点事件
            _toolBody.find('li.dodo-item').each(function() {
                var $that = $(this);
        ...
        winResize: function() {
            ...
            _win.on('resize', function() {
                ...
                switch (_config.renderType) {
                    case renderType.page:
                        $('.dodo-tab .layui-tab-content').height(currBoxHeight - 43);
                        break;
                    case renderType.iframe:
                        $('.dodo-tab .layui-tab-content iframe').height(currBoxHeight - 47);
                        break;
                }
            }).resize();
        },
